...
| `/api/*` | wikantik-rest | REST/JSON | 32 servlets registered in `web.xml` (incl. `POST /api/ingest` = derived-page document upload, `createPages`-gated) | `RestServletBase.checkPagePermission()` (ACL + policy grants) |
...
- **[KgInclusionPolicy.md](docs/wikantik-pages/KgInclusionPolicy.md)** — cluster-primary KG inclusion policy (`bin/kg-policy.sh`, default-exclude, `kg_include:` frontmatter override). **Known live defect (fixed in ClusterDeclarationDesign Phase 1):** `DefaultKgInclusionPolicy.lookupCluster` is exact-match, so a `parent/sub` cluster inherits no policy and is silently EXCLUDE'd from the Knowledge Graph.
