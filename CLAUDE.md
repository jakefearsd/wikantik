...
- **[ClusterDeclarationDesign.md](docs/wikantik-pages/ClusterDeclarationDesign.md)** — **DESIGN ONLY, not yet implemented (2026-08-15).** The hub page becomes the authoritative *declaration* of its cluster: a cluster exists iff exactly one page carries `type: hub` + scalar `cluster: <path>`. No new frontmatter fields. Sub-clusters are `parent/child` in the value, **one level, permanently** (depth limit lives only in `CLUSTER_SLUG_PATTERN`; every consumer must be depth-agnostic + segment-aware prefix matching, never `startsWith`). Exactly one blocking rule (duplicate declaration → 422 in `StructuralSpinePageFilter`); everything else WARNING → `/admin/drift`. **Consolidates three rival membership mechanisms into `cluster:`** — `hubs:` (473 pages, 89 already multi-membership, absent from `FrontmatterSchema`) and hub `related:`-as-membership are retired, `HubSyncFilter` is removed, hub `hasPart` becomes derived. 7 phases. **Phases 0 (content migration) and 1 (projection) are COMPLETE (2026-08-15)** — prod validates at 90 hubs / 90 clusters, 0 duplicate declarations, 0 headless, 0 clusterless, 28 sub-clusters; phases 0b and 2–5 are **not started**. Phase 1 shipped `ClusterPath` (wikantik-api) as the **single segment-aware cluster comparison — never use `startsWith`**, transitive cluster membership resolved at query time, deterministic hub tie-break (lowest slug), four `StructuralConflict.Kind` values, `cluster_status` on the page payload (carries `hub_declared` because Gson omits null `hub_slug`), and hub `hasPart` derived from real membership with `related` as the degraded-index fallback. **Permanently out of scope, not follow-ups:** depth > 1, and any filesystem-shaped export of the corpus. Decision recorded as ADR-0009.
