...
- **wikantik-admin-mcp**: Admin MCP server at `/wikantik-admin-mcp` — 26 tools — adds admin-bypass mirrors of query_nodes + search_knowledge so curators see freshly-created entities, plus `list_orphaned_kg_nodes` for finding degree-0 entities at scale, plus `list_retrieval_queries` (real-traffic query-log read). Reconciled 2026-05-14. See `com.wikantik.mcp.McpServerInitializer`.
