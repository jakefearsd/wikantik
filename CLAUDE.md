...
`bin/redeploy.sh` is the fast iteration path. It **does** run
`bin/db/migrate.sh` (idempotent — applies only pending `V*.sql`, as the app
role from `.env`), but it does **not** re-run template materialisation,
secrets validation, Tomcat-upgrade snapshot logic, or dev-user seeding. For
first-time setup, Tomcat upgrades, or anything that touches
`wikantik-custom.properties` / `ROOT.xml`, run `bin/deploy-local.sh` instead.
...
- **wikantik-mcp-core**: Shared MCP substrate — `McpTool`/`McpToolUtils`/`McpAudit`, endpoint bootstrap, the access filter, config, and the shared `QueryNodesTool`/`SearchKnowledgeTool` that both MCP endpoints register. Extracted in 2.3.0 to break the `wikantik-knowledge → wikantik-admin-mcp` module cycle. (Admin's 29-tool count = 27 tools declared in `wikantik-admin-mcp` + these 2.)
- **wikantik-coverage-report**: Aggregated JaCoCo cross-module coverage report. Build-time only — produces no runtime artifact.
- **wikantik-admin-mcp**: Admin MCP server at `/wikantik-admin-mcp` — 29 tools — adds admin-bypass mirrors of query_nodes + search_knowledge so curators see freshly-created entities, plus `list_orphaned_kg_nodes` for finding degree-0 entities at scale, plus `list_retrieval_queries` (real-traffic query-log read), plus `rename_cluster` (bulk cluster-path rewrite across every member; plan-by-default, applies on `confirm=true`), plus the Content Intelligence pair `list_content_opportunities` + `snooze_opportunity` (the ranked opportunity backlog and its decline path). Reconciled 2026-08-20. See `com.wikantik.mcp.McpServerInitializer`.
...
- **wikantik-insights** (pom name "Wikantik Content Intelligence"): Search-visibility fact store, content-opportunity rules, and effect measurement (`com.wikantik.insights.*` — `InsightsStore`/`JdbcInsightsStore`, `OpportunityEngine`, `EffectEvaluator`, `ExpectedCtrCurve`, `Backlog`). Deliberately depends on neither `wikantik-api` nor `wikantik-main` — the rule engine is a pure function of (rows, page facts, config), unit-testable without a wiki engine; page state arrives through a `PageFacts` port implemented by an adapter in `wikantik-main`. Admin surfaces: `/admin/insights/acquisition`, `/admin/insights/backlog`, `/admin/insights/ingest`. The reference implementation of the Simple Agility feedback-loop pattern (see the Simple Agility section above).
...
|----------|--------|----------|-------|------|
| `/wikantik-admin-mcp` | wikantik-admin-mcp | MCP (Streamable HTTP) | 29 write/analytics tools (incl. KG curation + admin-bypass reads + orphan-listing + `list_retrieval_queries` + `rename_cluster` + the Content Intelligence pair `list_content_opportunities`/`snooze_opportunity`) | `McpAccessFilter` (bearer token / API key) |
...
| `/api/*` | wikantik-rest | REST/JSON | 32 distinct servlets (33 url-patterns — `SelfApiKeysResource` carries two) registered in `web.xml` (incl. `POST /api/ingest` = derived-page document upload, `createPages`-gated) | `RestServletBase.checkPagePermission()` (ACL + policy grants) |
| `/admin/*` | wikantik-rest | REST/JSON | 29 distinct servlets (37 url-patterns) registered in `web.xml` (incl. `/admin/kg-policy/*`, the tamper-evident `/admin/audit*` log, `/admin/ontology/*`: rebuild + status + SHACL-conformance violations, `/admin/drift/*`: the drift burn-down dashboard + `/admin/drift/citations` bidirectional stale-citation views, `/admin/derived/*`: derived-page reflow + staleness status, `/admin/clusters/rename`: bulk cluster-path rewrite (plan-by-default, `confirm=true` applies), `/admin/connectors/*`: full connector CRUD + import + dry-run test + sync + runs/pages, with `/admin/connector-credentials/*` secret injection and `/admin/connector-oauth/gdrive/*` consent flow, and `/admin/insights/{acquisition,backlog,ingest}`: the Content Intelligence search-visibility + opportunity-backlog surfaces) | `AdminAuthFilter` (`AllPermission`) |
...
- `JDBCPlugin` (page-authored SQL against the wiki datasource) ships **disabled by default** behind `wikantik.plugin.jdbc.enabled` (checked before even the admin gate) — enable it only if you accept that a high-privilege DB role executes editor-supplied SQL.
- SSRF egress policy on connector fetches — `EgressGuard` (`wikantik-connectors`) rejects non-http(s) schemes and any host resolving to loopback/private/link-local/multicast/IPv6-ULA addresses, re-validated on every redirect hop; `wikantik.connectors.egress.allowPrivate=true` opts a deliberate internal crawl back in.
- Render caches skip viewer-dependent output — a render that consults the caller's session/ACLs (e.g. an ACL-aware `InsertPage`, `$username`) flags itself via `Context.VAR_VIEWER_SENSITIVE` and is excluded from the principal-less document/HTML caches, so it can't be cached for one user and served to another.
- Deserialization filtering — `ObjectInputFilter` whitelists on all `ObjectInputStream` usage
- MCP API-key scopes are rank-based (`ApiKeyService.Scope`): `mcp_read` (rank 1, read-only, confined to `/knowledge-mcp`) and `mcp` (rank 3, full admin `/wikantik-admin-mcp` surface, historical wire value `"mcp"` so pre-split keys stay full-admin) — `mcp_read ⊂ mcp`. Split in 2.4.18 so a leaked "read-only knowledge agent" token is no longer also a content-write + KG-curation credential. An intermediate `mcp_content` tier (page/KG read-write, no admin) is designed but not yet shipped — it needs per-tool enforcement threaded through the MCP session rather than a request-thread guard, since the SDK dispatches tool calls off-thread.
