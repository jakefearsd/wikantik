...
| `/knowledge-mcp` | wikantik-knowledge | MCP (Streamable HTTP) | 20 read-only retrieval + Knowledge Graph + Page Graph structural-spine + agent-projection + batched-read tools when fully wired (incl. `get_ontology` + `sparql_query` + **`list_stale_citations`**, plus **`assemble_bundle`** = RAG context bundle, present when retrieval is wired) | `KnowledgeMcpAccessFilter` (same scheme) |
