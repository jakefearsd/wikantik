...
| `wikantik-rest` | REST/JSON API — 32 servlets on `/api/*` (33 `url-pattern` entries — `SelfApiKeysResource` maps two; incl. `/api/bundle` context bundle and `/api/briefing` session briefing) and 29 servlets on `/admin/*` (37 `url-pattern` entries; incl. `/admin/drift/*`, `/admin/clusters/rename`, `/admin/connectors/*`, `/admin/ontology/*`, `/admin/insights/*`); also hosts the public RDF surface (`/sparql`, `/id/*`, `/export/*`) |
