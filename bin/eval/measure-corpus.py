...
from bundle_eval_common import load_corpus, fetch_bundle, sublist
...
def main():
    ...
    for qd in corpus:
        qid = qd["qid"]
        try:
            secs = fetch_bundle(BASE, qd["query"])
        except Exception as e:
            print(f"  {qid} fetch fail: {e}", flush=True)
            secs = []
        for cid, ghp in qd["golds"]:
            hit12 = any(s[0] == cid and sublist(ghp, s[1]) for s in secs[:12])
            if not hit12:
                misses.append(qid)
            for n in NS:
                h = 1.0 if any(s[0] == cid and sublist(ghp, s[1]) for s in secs[:n]) else 0.0
