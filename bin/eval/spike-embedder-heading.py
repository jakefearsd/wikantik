#!/usr/bin/env python3
"""
Cheap contextual-embedding test: raw chunk vs HEADING-PREPENDED chunk (same 0.6B embedder).

Isolates one structural change: embed `<heading path> — <chunk text>` instead of the raw
chunk, so a context-stripped fragment carries its section breadcrumb. Baseline = the existing
DB vectors (raw-chunk 0.6B, production-exact); treatment = re-embed (heading + text) with 0.6B.
Same query embedding (0.6B + instruction prefix) for both, so the ONLY difference is the
document side. Reports max-chunk shortlist recall@N.

Self-check: the raw column should reproduce production ~0.24@5 / ~0.47@12.
Light (0.6B); requires PG* env + the 0.6B embedder.
"""
...
_qc={}; _dc={}; _lat=[]
def embed(text, instr=False):
    t=(INSTR+text) if instr else text
    if not instr:
        if t[:CHUNK_CAP+80] in _dc: return _dc[t[:CHUNK_CAP+80]]
    req=urllib.request.Request(EMB_URL,data=json.dumps({"model":M06,"prompt":t[:CHUNK_CAP+200]}).encode(),
        headers={"Content-Type":"application/json"},method="POST")
    t0=time.time()
    with urllib.request.urlopen(req,timeout=120) as r: v=json.load(r)["embedding"]
    if not instr: _lat.append(time.time()-t0); _dc[t[:CHUNK_CAP+80]]=v
    return v
def qembed(query):
    if query in _qc: return _qc[query]
    v=embed(query,instr=True); _qc[query]=v; return v
def cos(a,b):
    dot=sa=sb=0.0
...
def fetch(slugs):
    ...
    pages={}  # page -> section(norm segs) -> list of (vec06_raw, heading_str, text)
    for ln in out.splitlines():
        ...
        pages.setdefault(page,{}).setdefault(segs,[]).append((vec,hp.strip(),txt))
    return pages
...
def main():
    ...
    res={"raw":{n:[] for n in NS},"head":{n:[] for n in NS}}
    for qi,q in enumerate(corpus,1):
        qv=qembed(q["query"])
        secs=[]  # (page, segs, raw_score, head_score)
        for s in cand[q["qid"]]:
            for segs,chunks in pages.get(s,{}).items():
                raw=max(cos(qv,v) for v,_,_ in chunks)
                head=max(cos(qv,embed(hd+" — "+tx)) for _,hd,tx in chunks)
                secs.append((s,segs,raw,head))
        for key,idx in (("raw",2),("head",3)):
    ...
    print("\nHeading-prepended embeddings — max-chunk shortlist recall@N (same 0.6B)\n")
    print(f"{'document form':<24}"+"".join(f"{('recall@'+str(n)):>11}" for n in NS))
    print("-"*(24+11*len(NS)))
    print(f"{'raw chunk (prod)':<24}"+"".join(f"{avg(res['raw'][n]):>11.3f}" for n in NS))
    print(f"{'heading + chunk':<24}"+"".join(f"{avg(res['head'][n]):>11.3f}" for n in NS))
    print(f"\n0.6B doc embed p50={p50*1000:.0f}ms (n={len(lat)})")
