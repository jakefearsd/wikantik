...
from bundle_eval_common import load_corpus, load_chunk_section_map, rrf_fuse, group_to_sections, recall_at
...
def base_sections(dense, bm25std, cmap):
    return group_to_sections(rrf_fuse(dense, bm25std, BASE_DW, BASE_BW, BASE_K, BASE_TR), cmap, "first")
...
def inject(base_keys, bm25code, cmap, drank, has_sym, cfg):
    ...
    code_secs = group_to_sections(bm25code, cmap, "first", with_scores=True)  # [(key,score)] best chunk per section, bm25 order
...
def main():
    ...
    for name, path in CORPORA.items():
        rows = []
        for qd in load_corpus(path):
            dense, bm25std, bm25code = fetch(qd["query"])
            rows.append((qd["qid"], qd, base_sections(dense, bm25std, cmap), bm25code, dense, has_symbol(qd["query"])))
        cache[name] = rows
    def evalu(name, cfg):
        ov = {n: [] for n in NS}
        for qid, qd, basek, bm25code, dense, sym in cache[name]:
            ...
            for n in NS: ov[n].extend(recall_at(secs, qd["golds"], n))
