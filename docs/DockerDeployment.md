...
Defaults below are copied from `docs/ConfigurationReference.md`; that page is authoritative.
...
| `WIKANTIK_SSO_OIDC_DISCOVERY_URI` | *(blank)* | `wikantik.sso.oidc.discoveryUri` | Provider OIDC discovery URL, e.g. `https://accounts.google.com/.well-known/openid-configuration`. |
| `WIKANTIK_SSO_OIDC_CLIENT_ID` | *(blank)* | `wikantik.sso.oidc.clientId` | OAuth client id from the provider console. |
| `WIKANTIK_SSO_OIDC_CLIENT_SECRET` | *(blank)* | `wikantik.sso.oidc.clientSecret` | OAuth client secret — keep in `.env.prod`, never in git. |
...
| `WIKANTIK_EMBEDDING_BASE_URL` | `http://inference.jakefear.com:11434` | `wikantik.search.embedding.base-url` | Dense-retrieval embedding service base URL override, e.g. `http://ollama-embed:11434` for the cloud overlay's sidecar. |
