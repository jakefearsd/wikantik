...
- **HashMap lookups at 617 combined** (3 frames) are scattered but cumulatively significant — probably `HybridFuser`'s score map, the new `relatedByPage` map, the `byName` lookup map in `applyHybridRerank`, or `QueryEmbedder`'s cache key map. Worth identifying with a deeper capture.
