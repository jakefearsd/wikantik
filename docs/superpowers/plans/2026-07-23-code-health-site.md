...
**Architecture:** Extend the existing root `<reporting>` block with the missing reports (taglist, dependency-updates, CPD, aggregate coverage). Add a **module coupling** page from `depgraph-maven-plugin` (generated in `bin/site.sh` before `mvn site`, SVG via graphviz with a `.dot` fallback). Replace the generic landing page with a hand-written dashboard. Wrap the two-phase build (coverage build → `mvn site site:stage`) in `bin/site.sh`, and publish `target/staging/` with `bin/deploy-site.sh` (a clone of `bin/deploy-marketing.sh` targeting `/var/www/www.wikantik.com/site/`).
...
### Task 5: Module coupling — FOLDED INTO TASK 7

**Superseded during the Task 1 spike.** depgraph-maven-plugin 4.0.3 has no
`graphFormat=svg` (the enum is `dot|puml|json|text|gml|tgf`); an SVG is produced
by rendering the `.dot` with graphviz `dot -Tsvg`, which needs graphviz present.
Binding this to `pre-site` created an awkward intra-`mvn site` ordering problem
(the SVG must exist *after* pre-site but *before* the `site` goal copies
`generated-site/resources`). Cleaner and more robust: generate the graph in
`bin/site.sh` **before** `mvn site` runs, graphviz-optional. So the coupling
`.dot` + SVG generation moves into Task 7's `bin/site.sh` (see its Step 2), and
**no `pom.xml` change is needed** for coupling. The `.dot` graph content was
verified in the spike: 72 inter-module `com.wikantik*` edges.

Task 6's `coupling.md` still embeds `images/module-coupling.svg` and links the
`.dot` fallback — unchanged.
...
```
Notes:
- Confirm `bin/run-tests.sh --parallel 4` forwards extra args like `-Pcoverage`; if it does not accept a trailing profile arg, replace that line with the module-by-module IT-coverage invocation the `wikantik-coverage-report` module documents.
- The coupling block uses depgraph's default output path (`target/dependency-graph.dot`, verified in the Task 1 spike) rather than relying on the plugin's `outputDirectory`/`outputFileName` CLI properties; it copies to a stable name under the site resources dir.
...
- Coupling = depgraph module graph + PMD design rules, no JDepend → Tasks 1, 6, 7 (old Task 5 folded into Task 7 after the spike found `graphFormat=svg` invalid) ✓
