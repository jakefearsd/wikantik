...
> **Status: EXECUTED 2026-08-22** — all phases landed on `main` the same day (Phase 0 → Wave 1 → Wave 2 → Phase 3);
> J-1 unfrozen at zero violations, test-DDL baseline down to the one justified `JDBCPluginCITest` entry.
> Deviations from the text below: `insertReturningKey`, `withConnection` and `supportsTransactions` were added to
> `Jdbc` mid-flight (generated keys, no-transaction connection lending, the auth startup probe); `EdgeProjector`
> was a grep false positive (Jena, not JDBC); V058 (`api_keys_scope_chk` admits `mcp_read`) and the hub-reader
> `related_to` fix were shipped-defect finds, not planned tasks. Record: ADR-0010, CHANGELOG [Unreleased].
...
- [x] Add module `wikantik-jdbc` after `wikantik-util` in the root reactor `<modules>`; parent
...
- [x] **Move** (git mv) `JdbcSupport.java` and `Transactions.java` to
...
- [x] Promote `SqlBinder`, `RowMapper`, `TransactionBody` to **top-level public interfaces** in
...
- [x] Write `com.wikantik.jdbc.Jdbc` — **final class, composition** (`new Jdbc(DataSource)`), public:
...
- [x] Re-implement `JdbcSupport` as a thin `extends` convenience over a private `Jdbc` field
...
- [x] Unit tests in `wikantik-jdbc` (H2 is fine *here* and only here — this module tests the
...
- [x] `wikantik-main/pom.xml` depends on `wikantik-jdbc`. `mvn -q -pl wikantik-jdbc,wikantik-main
...
- [x] Commit: `build(jdbc): wikantik-jdbc module — Jdbc primitive, JdbcSupport/Transactions moved`.
...
- [x] `wikantik-war/pom.xml`: add test-scope `archunit-junit5` (version from root `archunit.version`),
...
- [x] `JdbcAccessArchTest` with `@AnalyzeClasses(packages = "com.wikantik", importOptions =
...
- [x] Sanity-check the store after the first freeze: it must list the ~55 production violators and
      **no test classes** (`TestEngine`, `*Test`). If test-jars leak in, add an `ImportOption`
      excluding `-tests.jar` locations.
- [x] `TestSchemaSingleSourceTest` (plain JUnit, in the same package): walk `<repoRoot>/*/src/test/java`
...
- [x] `mvn -q -pl wikantik-war test -Dtest='JdbcAccessArchTest,TestSchemaSingleSourceTest'` green.
- [x] Commit: `test(arch): J-1 JDBC-access ratchet + test-DDL single-source ratchet (baselined)`.
...
- [x] `com.wikantik.jdbc.testing.MigrationApplier`: locate repo root (walk up from `user.dir` to the
...
- [x] `com.wikantik.jdbc.testing.PostgresTestDb`: per-JVM singleton over
...
- [x] `com.wikantik.jdbc.testing.FaultInjectingDataSource`: wraps a DataSource; `failOn(n,
...
- [x] Split `postgresql-test.sql`: everything that is seed *data* (the "test seed data" section —
...
- [x] Run the full Postgres-backed suite in wikantik-main: `bin/agent-build.sh start pgfix -- mvn
...
- [x] Commit: `test(jdbc): PostgresTestDb applies bin/db/migrations; replaces PostgresTestContainer`
      (body lists any drift findings).

### Task 0.4 — Orchestrator gate before waves

- [x] `bin/agent-build.sh start unit -- mvn clean install -DskipITs` → SUCCESS (this also installs
      `2.4.20-SNAPSHOT` artifacts **including both test-jars** into `~/.m2`, which the worktree
      agents resolve `-pl` builds against).
- [x] `docker pull pgvector/pgvector:pg17` warm; Colima up (`colima status`; memory
...
- [x] **T1.1** `citation/CitationRepository` + its tests (`TestCitationDb`/`CitationEdgesIT` → PostgresTestDb).
- [x] **T1.2** `drift/DriftSnapshotRepository` + test.
- [x] **T1.3** `knowledge/KgProposalRepository` (already `KgJdbcSupport`; replace the hand-rolled block at ~413 with `inTransaction`) + test.
- [x] **T1.4** `comments/*`: `CommentStore` (delete private `inTransaction`), `PageOwnerService`, `MentionService`, `mentions/MentionFeedDao` + their H2 tests (6 files). `CommentStoreTransactionTest` becomes the FaultInjecting test.
- [x] **T1.5** `auth/*`: `AbstractJDBCDatabase.runInTransaction` delegates to `Jdbc.inTransaction` (keep `WikiSecurityException` wrapping + `supportsCommits`); `JDBCUserDatabase`, `JDBCGroupDatabase`, `DatabasePolicy`, `apikeys/ApiKeyService` + tests (ApiKeyService H2 → PG).
- [x] **T1.6** `search/embedding/{EmbeddingIndexService,PgVectorBackfillCli,BootstrapEmbeddingIndexer}` + `search/hybrid/PgVectorChunkVectorIndex` + tests (streaming reads use `forEachRow`; batches use `batch`).
- [x] **T1.7** `audit/JdbcAuditRepository` (partition DDL via `execute(conn, ddl)` inside the same transaction) + test.
- [x] **T1.8** `knowledge/extraction/ChunkEntityMentionRepository` + `extractcli/KgPolicyCli` + tests (two modules, disjoint).
...
- [x] **T2.1** `wikantik-connectors`: the 5 stores + 5 H2 tests; pom gets `wikantik-jdbc` (compile) and its test-jar + testcontainers (test). Update the connectors module's own README/Javadoc if it claims H2.
- [x] **T2.2** `wikantik-insights/JdbcInsightsStore`: pom gets `wikantik-jdbc`; fold `JdbcInsightsStoreTest` (H2, self-limited) into `JdbcInsightsStorePostgresTest` (switch it from its private `@Container` to `PostgresTestDb`), delete the H2 file. Report in the commit body that insights now depends on `wikantik-jdbc` (CLAUDE.md's "depends on neither wikantik-api nor wikantik-main" sentence stays true; orchestrator adds the jdbc note in Phase 3).
- [x] **T2.3** `knowledge/{HubDiscoveryRepository,HubProposalRepository,KgEdgeAuditRepository,KgRejectionRepository,MentionIndex,DefaultKnowledgeGraphService}` + tests.
- [x] **T2.4** `knowledge/embedding/*`, `knowledge/judge/JdbcKgJudgeTimeoutRepository`, `knowledge/eval/{BundleEvalRunDao,RetrievalQualityDao}` + tests (`RetrievalQualityDaoTest` H2 → PG).
- [x] **T2.5** `knowledge/querylog/*` (3) + `kgpolicy/*` (2) + tests (fold `JdbcQueryLogServiceTest` into the Postgres one; `JdbcQueryLogReaderTest` H2 → PG).
- [x] **T2.6** `search/hybrid/{InMemoryChunkVectorIndex,LuceneBm25ChunkIndex,LuceneHnswChunkVectorIndex}` + tests (read-only JDBC; pure mechanical).
- [x] **T2.7** `pagegraph/spine/*` tests only: `PageCanonicalIdsDaoTest`, `PageVerificationDaoTest`, `TrustedAuthorsDaoTest` H2 → PostgresTestDb (the DAOs are already on `JdbcSupport`).
- [x] **T2.8** `rest/AdminPolicyResource`: move its four statements into `com.wikantik.auth.DatabasePolicy` (which already owns `policy_grants` reads) as `listGrants/insertGrant/updateGrant/deleteGrant` on `Jdbc` (`INSERT … RETURNING id` via `queryOne` replaces `RETURN_GENERATED_KEYS`); servlet calls them; `AdminPolicyResourceDbTest` follows. Plus `observability/DatabaseHealthCheck` (`Jdbc.ping()`), `ontology/projection/EdgeProjector`, `extractcli/{BootstrapExtractionCli,JudgeExperimentCli}` (one statement each).
- [x] **T2.9** Orphan H2-DDL tests → PostgresTestDb: `derived/ConnectorWiringHelperTest`, `derived/ConnectorConfigServiceTest`, `connectors/ConnectorSyncEndToEndTest` (main), `search/subsystem/BundleSourcesWithoutEmbedderTest`, `knowledge/eval/{RetrievalQualitySmokeTest,DefaultRetrievalQualityRunnerTest}`, `rest/CommentThreadResourceTest`.
...
- [x] **T3.1** Convert J-1 from `freeze(...)` to a plain rule (delete the store entry/file) — the
      ratchet has reached zero and must stay there. Keep the allowlist comment for `JDBCPlugin`.
- [x] **T3.2** Schema single-source: delete `bin/db/postgresql.ddl`, `postgresql-hub.ddl`,
...
- [x] **T3.4** Docs: `CLAUDE.md` — module table entry for `wikantik-jdbc`; a one-paragraph
...
- [x] **T3.5** Canonical gate: `bin/run-tests.sh --parallel 4` → green (fresh log mtimes).
- [x] **T3.6** Memory note (`duplication_hides_defects` already exists): link this plan as the
      worked example.
