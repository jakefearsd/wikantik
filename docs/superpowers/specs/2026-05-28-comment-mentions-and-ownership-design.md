...
1. `directMentions = resolve(parse(body))` — self-mentions are kept (users can intentionally @-mention themselves, e.g. as a personal todo marker).
...
1. `oldSet = resolve(parse(oldBody))`, `newSet = resolve(parse(newBody))` (both including self-mentions).
...
- **Mentioning yourself:** allowed — self-mentions are recorded as a regular direct mention row (useful for "@me todo" markers, and the only way for a sole admin to round-trip the feature in test).
