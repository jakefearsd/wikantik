...
1.  **Deterministic vs. Randomized Analysis:** Amortized analysis, as typically taught, assumes a deterministic sequence of operations. If the sequence is generated by a truly random process, the *expected* analysis (using probability theory) is usually the correct tool, which is distinct from amortized analysis.
