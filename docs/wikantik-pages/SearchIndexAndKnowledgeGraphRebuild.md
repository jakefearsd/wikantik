---
canonical_id: 01KQE8VJHXHV6AGEBV8934FX6F
related:
- WikantikKnowledgeGraphAdmin
title: Search Index and Knowledge Graph Rebuild
date: 2026-04-26T00:00:00Z
status: active
summary: Operator runbook for the four-phase end-to-end rebuild of chunks, Lucene,
  embeddings, and the entity-extraction layer. Covers the bin/kg-rebuild.sh orchestration
  script, the optional KG reset step, and how to use the rebuild as an experiment
  harness for tuning chunker config, prefilter thresholds, and extractor models.
tags:
- administration
- operations
- search
...
0curation work. Phases 1, 2, 4 skipped; phase 3 runs alone.
...
| **Backend-specific** | | |
| `WIKANTIK_EXTRACT_PG_PASSWORD` | (none) | Read by `bin/kg-extract.sh` if `--jdbc-password-env` targets this variable. |
| `ANTHROPIC_API_KEY` | (none) | Required for the `claude` backend. |
...
**Hub clustering empty after `--purge-kg`** — by design. The hub-clustering subsystem writes its own data; phase 4's extraction doesn't repopulate it. It will fill back up on the next page-save cycle or when the hub-discovery service runs.
