...
$$\Delta U = Q - W$$
The change in internal energy ($\Delta U$) of a closed system equals the heat added ($Q$) minus the work done by the system ($W$).

### 1.2 Second Law: Entropy and Directionality
Heat cannot spontaneously flow from a colder body to a hotter body. This law limits the theoretical efficiency of all heat engines and refrigerators.
$$\Delta S_{total} \geq 0$$

## 2. Heat Pump Mechanics
...
### 3.1 COP Calculation
$$COP_{heating} = \frac{Q_{hot}}{W_{in}}$$
$$COP_{cooling} (EER) = \frac{Q_{cold}}{W_{in}}$$

### 3.2 The Carnot Limit (Maximum Theoretical Efficiency)
The maximum possible COP is determined by the absolute temperatures ($T$ in Kelvin) of the source and sink:
$$COP_{Carnot, heating} = \frac{T_{hot}}{T_{hot} - T_{cold}}$$
*Implication:* As the temperature difference ($\Delta T$) between outside and inside increases, the COP drops significantly. A heat pump with a COP of 4.0 delivers 4 units of heat for every 1 unit of electricity consumed (400% efficiency).
