...
CFG = SimpleNamespace(provider="ollama", anthropic_key=None, agent_model="m",
...
def test_cold_arm(monkeypatch):
    monkeypatch.setattr(arms.ollama_client, "complete",
                        lambda *a, **k: {"message": {"content": "cold ans. Sources: [HybridRetrieval]"}})
    r = arms.cold(CFG, Q)
...
def test_grounded_bundle_arm(monkeypatch):
    ...
    monkeypatch.setattr(arms.ollama_client, "complete",
                        lambda *a, **k: {"message": {"content": "grounded"}})
...
def test_grounded_mcp_arm(monkeypatch):
    ...
    monkeypatch.setattr(arms.ollama_client, "run_tool_loop",
...
def test_arms_honor_anthropic_provider(monkeypatch):
    """With provider=anthropic the arms dispatch to the Anthropic client instead."""
    acfg = SimpleNamespace(provider="anthropic", anthropic_key="k", agent_model="claude-sonnet-4-6",
                           base_url="http://x", ollama_base_url="http://o", mcp_key="mk",
                           lexical=False, max_tool_iters=4)
    monkeypatch.setattr(arms.anthropic_client, "complete",
                        lambda *a, **k: {"content": [{"type": "text", "text": "anthropic ans"}]})
    r = arms.cold(acfg, Q)
    assert "anthropic ans" in r["answer"]
