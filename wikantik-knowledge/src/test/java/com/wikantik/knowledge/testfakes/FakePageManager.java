...
public class FakePageManager implements PageManager {
    ...
    private static final class StoredPage {
        ...
        synchronized Page toPage( final int version ) {
            if ( cached == null ) {
                cached = mock( Page.class );
                when( cached.getName() ).thenReturn( name );
                when( cached.getAuthor() ).thenReturn( author );
                when( cached.getLastModified() ).thenReturn( lastModified );
                when( cached.getVersion() ).thenReturn( version );
            }
            return cached;
        }
    }
