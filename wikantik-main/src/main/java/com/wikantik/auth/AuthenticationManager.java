...
public interface AuthenticationManager extends Initializable {
     ...
     * {@link com.wikantik.auth.authorize.WebContainerAuthorizer#isContainerAuthorized()},
     ...
     * {@link com.wikantik.event.WikiSecurityEvent#LOGIN_AUTHENTICATED} with two parameters: a Principal representing the login principal,
     * and the current Session. In addition, if the authorizer is of type WebContainerAuthorizer, this method iterates through the
     * container roles returned by {@link com.wikantik.auth.authorize.WebContainerAuthorizer#getRoles()}, tests for membership in each
     ...
     * {@link com.wikantik.event.WikiSecurityEvent#LOGIN_ASSERTED} with two parameters: <code>WikiPrincipal(<em>cookievalue</em>)</code>,
     ...
     * {@link com.wikantik.event.WikiSecurityEvent#LOGIN_ANONYMOUS} with two parameters: <code>WikiPrincipal(<em>remoteAddress</em>)</code>,
     * and the current Session </li>
     * </ul>
     *
     ...
     * @throws com.wikantik.auth.WikiSecurityException if the user cannot be logged in for any reason
     * @since 2.3
     */
     ...
     * the {@link com.wikantik.auth.login.UserDatabaseLoginModule} class will be used. When the LoginModule's <code>initialize</code>
     ...
     * @throws com.wikantik.auth.WikiSecurityException if the Authorizer or UserManager cannot be obtained
     */
