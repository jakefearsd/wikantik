 ...
 * {@link com.wikantik.auth.AuthenticationManager} consults the configured Authorizer to determine which additional
 * {@link com.wikantik.auth.authorize.Role} principals should be added to the user's Session. To determine which roles should be
 * injected, the Authorizer is queried for the roles it knows about by calling {@link com.wikantik.auth.Authorizer#getRoles()}. Then,
 * each role returned by the Authorizer is tested by calling {@link com.wikantik.auth.Authorizer#isUserInRole(Session, Principal)}.
 * If this check fails, and the Authorizer is of type WebAuthorizer, AuthenticationManager checks the role again by calling
 * {@link com.wikantik.auth.authorize.WebAuthorizer#isUserInRole(jakarta.servlet.http.HttpServletRequest, Principal)}).
