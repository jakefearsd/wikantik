...
public class WebContainerAuthorizer implements WebAuthorizer  {
     ...
     * {@link com.wikantik.ui.WikiServletFilter#doFilter(jakarta.servlet.ServletRequest, jakarta.servlet.ServletResponse, jakarta.servlet.FilterChain)}
     ...
     * in particular {@link com.wikantik.auth.login.WebContainerLoginModule}}.
     ...
     * @param role the role to check
     * @return <code>true</code> if the user is considered to be in the role, <code>false</code> otherwise
     * @see com.wikantik.auth.Authorizer#isUserInRole(com.wikantik.api.core.Session, java.security.Principal)
     */
    @Override
     ...
     * @see com.wikantik.auth.Authorizer#initialize(Engine, Properties)
     */
    @Override
    ...
    public boolean isConstrained( final String url, final Role role ) {
        ...
        // Role ALL is always constrained.
        // Otherwise we need a non-empty roles list AND a constraint that appears in both lists.
        return role.equals( Role.ALL )
                || ( !roles.isEmpty()
     ...
     * {@link com.wikantik.auth.authorize.Role#AUTHENTICATED} is required
