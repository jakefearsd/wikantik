...
<%
  ...
  if( !TextUtil.isPositive(wiki.getWikiProperties().getProperty("jspwiki-x.securityconfig.enable")) )
  {
      %>
      <head>
        <title><wiki:Variable var="applicationname" default="Wikantik" />: Wikantik Security Configuration Verifier</title>
        <base href="../"/>
      ...
      <body class="container">
         <h1>Disabled</h1>
         <p>Wikantik SecurityConfig UI has been disabled.  This page could reveal important security
...
<h1>Wikantik Security Configuration Verifier</h1>

<p>This page examines Wikantik's security configuration and tries to determine if it is working the way it should. Although Wikantik comes configured with some reasonable default configuration settings out of the box, it's not always obvious what settings to change if you need to customize the security... and sooner or later, just about everyone does.</p>

<p>This page is dynamically generated by Wikantik. It examines the authentication, authorization and security policy settings. When we think something looks funny, we'll try to communicate what the issue might be, and will make recommendations on how to fix the problem.</p>
...
<p>in your wikantik-custom.properties.
</p>
</blockquote>
...
<p>Wikantik wires up its own JAAS to define the authentication process, and does not rely on the JRE configuration. By default, Wikantik configures its JAAS login stack to use the UserDatabaseLoginModule. You can specify a custom login module by setting the <code>jspwiki.loginModule.class</code> property in <code>wikantik.properties</code>.</p>
...
<p>Wikantik's authorizes user actions by consulting a standard Java 2 security policy file. By default, Wikantik installs its local security policy file at startup time. This policy file is independent of your global, JVM-wide security policy, if you have one. When checking for authorization, Wikantik consults the global policy first, then the local policy.</p>
...
And as an additional check, we will try to load each <code>Permission</code> class into memory to verify that Wikantik's classloader can find them.</p>
...
<%
  ...
  {
%>
    <p>Note: Wikantik's Policy file parser is stricter than the default parser that ships with the JVM. If you encounter parsing errors, make sure you have the correct comma and semicolon delimiters in your policy file <code>grant</code> entries. The <code>grant</code> blocks must follow this format:</p>
    <blockquote>
    ...
    <p>Note: Wikantik versions prior to 2.4.6 accidentally omitted commas after the <code>signedBy</code> entries, so you should fix this if you are using a policy file based on a version earlier than 2.4.6.</p>
<%
  }
%>
...
<p>Now comes the <em>really</em> fun part. Using the current security policy, we will test the PagePermissions each Wikantik role possesses for a range of pages. The roles we will test include the standard Wikantik roles (Authenticated, All, etc.) plus any others you may have listed in the security policy. In addition to the PagePermissions, we will also test the WikiPermissions. The results of these tests should tell you what behaviors you can expect based on your security policy file. If we had problems finding, parsing or verifying the policy file, these tests will likely fail.</p>
...
%>
    ...
    <p>Here is how your web container will control role-based access to some common Wikantik actions and their assocated JSPs. These restrictions will be enforced even if your Java security policy is more permissive.</p>
...
<p>The user database stores user profiles. It's pretty important that it functions properly. We will try to determine what your current UserDatabase implementation is, based on the current value of the <code>jspwiki.userdatabase</code> property in your <code>wikantik.properties</code> file. In addition, once we establish that the UserDatabase has been initialized properly, we will try to add (then, delete) a random test user. If all of these things work they way they should, then you should have no problems with user self-registration.</p>
...
<p>The group database stores wiki groups. It's pretty important that it functions properly. We will try to determine what your current GroupDatabase implementation is, based on the current value of the <code>jspwiki.groupdatabase</code> property in your <code>wikantik.properties</code> file. In addition, once we establish that the GroupDatabase has been initialized properly, we will try to add (then, delete) a random test group. If all of these things work they way they should, then you should have no problems with wiki group creation and editing.</p>
